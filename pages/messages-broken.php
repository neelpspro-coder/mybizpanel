...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'send':
                try {
                    $stmt = $pdo->prepare("INSERT INTO messages (id, sender_id, content, created_at) VALUES (?, ?, ?, NOW())");
                    $stmt->execute([
                        generateId(),
                        $_SESSION['user_id'],
                        trim($_POST['content'])
                    ...
                    echo "<script>document.addEventListener('DOMContentLoaded', function() { showAutoNotification('Message envoyé dans le canal général', 'success'); });</script>";
                    
                    // Redirection pour éviter la re-soumission
                    echo "<script>setTimeout(function() { window.location.href = '?page=messages'; }, 100);</script>";
                } catch (Exception $e) {
...
<div class="space-y-6">
    ...
    <div class="card overflow-hidden">
        ...
        <div class="p-4 border-t border-gray-200 bg-white">
            <form method="POST" class="flex space-x-3" onsubmit="return validateMessage()">
                <input type="hidden" name="action" value="send">
                <div class="flex-1">
                    <textarea id="message-input" name="content" required placeholder="Tapez votre message ici..." 
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-violet-500 resize-none"
                ...
                <div>
                    <button type="submit" class="btn-primary px-6 py-3 rounded-lg h-full flex items-center hover:shadow-lg transition-all">
        ...
        .then(data => {
            if (data.success && data.new_messages && data.new_messages.length > 0) {
                ...
                data.new_messages.forEach(message => {
                    ...
                    // Si ce n'est pas notre message et la page n'est pas visible, notifier
